...
#include "snappy_interfaces/msg/detection_array.hpp"
#include "snappy_interfaces/msg/object_detection.hpp"
#include "snappy_interfaces/msg/bounding_box2_d.hpp"
#include "snappy_interfaces/msg/polygon2_d.hpp"
#include <geometry_msgs/msg/point32.hpp>
#include "snappy_computer_vision/detection_classes.hpp"
...
public:
    ...
    : Node("bottom_camera_vision", options)
    {
        ...
        engine_path_ = this->declare_parameter<std::string>(
            "engine_path", "/home/kraken/Desktop/dfc_rs_26.engine");
        num_classes_ = this->declare_parameter<int>("num_classes", 8);
        camera_name_ = this->declare_parameter<std::string>("camera_name", "bottom");
        ...
        RCLCPP_INFO(get_logger(), "  capture_enabled: %s", capture_enabled_ ? "true" : "false");        RCLCPP_INFO(get_logger(), "  image_capture_directory: %s", image_capture_dir_.c_str());        RCLCPP_INFO(get_logger(), "  video_capture_directory: %s", video_capture_dir_.c_str());
        ...
        // Publish detections
        detection_pub_ = this->create_publisher<snappy_interfaces::msg::DetectionArray>(
            "/d405/detections", 10);
...
private:
    ...
    void start_video_capture(const cv::Mat &frame)
    {
        ...
        video_path_ = (std::filesystem::path(video_capture_dir_) / ("bottom_camera_" + timestamp.str() + ".mp4")).string();
    ...
    // save bottom camera image after inference
    void save_image(const cv::Mat &image, const rclcpp::Time &timestamp)
    {
        ...
        const std::string path =
            ...
            ("bottom_camera_" + std::to_string(timestamp.seconds()) + ".jpg");
    ...
    void publish_detections(const std::vector<Detection> & detections,
                            ...
                            float inference_ms)
    {
        auto msg = snappy_interfaces::msg::DetectionArray();
        ...
        for (const auto & det : detections) {
            ...
            snappy_interfaces::msg::Polygon2D poly;
    ...
    rclcpp::Publisher<snappy_interfaces::msg::DetectionArray>::SharedPtr detection_pub_;
    ...
    int         num_classes_     = 80;
    std::string camera_name_ = "bottom";
    ...
    // Video capture
    std::string image_capture_dir_ = "/home/snappy_data/bottom_camera/images";
    std::string video_capture_dir_ = "/home/snappy_data/bottom_camera/videos";
    double video_capture_fps_ = 30.0;
    std::string video_path_;
    cv::VideoWriter video_writer_;
    std::mutex video_mutex_;

    // DFC (bottom camera) classes, indexed by class_id (see detection_classes.hpp).
    // class_name_for() returns "unknown" for out-of-range ids.
    std::vector<std::string> class_names_ = snappy_computer_vision::kBottomDetectionClasses;
};
